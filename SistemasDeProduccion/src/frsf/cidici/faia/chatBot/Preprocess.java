...
public class Preprocess {
	public static String destinoChat = new String();
...
//			cadena = cadena.substring(0, cadena.indexOf("?")) + " " + cadena.substring(cadena.indexOf("?"));
		...
		// sacamos las letras de longitud 1
		for (int i = 0; i < separada.length; i++)
			if (separada[i].length() > 1)
				separadas.add(separada[i]);

		// identificamos el destino de comollegar
		...
		while (iter.hasNext()) {
			destinoChat = destinoChat.concat(iter.next()) + " ";

		destinoChat = Character.toUpperCase(destinoChat.charAt(0)) + destinoChat.substring(1);
		}
		
		if(destinoChat.length() > 0)
			Messenger.conexionViejo(destinoChat);

		
//		System.out.println("destinooooo:" + destinoChat);
	...
	private static void juntarMaterias(String[] separada) {
		...
		for (int i = 0; i < separada.length; i++) {
			if (separada.length != 1) {
				if (separada[i].equals("sistemas") && separada[i + 1].equals("de")
						&& separada[i + 2].equals("gestion")) {
					separada[i] = separada[i].concat(separada[i + 2]);
					separada[i + 1] = " ";
					separada[i + 2] = " ";
					...
					separada[i] = separada[i].concat(separada[i + 1]);
					separada[i + 1] = " ";
				} else if (separada[i].equals("analisis") && separada[i + 1].equals("matematico")
						&& separada[i + 2].equals("ii")) {
					separada[i] = separada[i].concat(separada[i + 1]);
					separada[i] = separada[i].concat(separada[i + 2]);
					separada[i + 1] = " ";
					separada[i + 2] = " ";
